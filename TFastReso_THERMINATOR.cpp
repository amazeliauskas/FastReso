...
status = gsl_integration_qag (&functhermal, 0, 1, 1e-4,1e-4, fLimit,GSL_INTEG_GAUSS15 ,fWorkspacethermal, &result, &error); 
    ...
    for (int i=0; i < getParticle(pi)->getNpbar(); i++){
      ...
      for (int j=0; j < grid_params::fNf ; j++){
        getParticle(pi)->addFj(j,i, get_initial_Fj(j, Ebar,Tfo,QMu, getParticle(pi)->getType(), m, Cs2));
      }
    }
  gsl_integration_workspace_free (fWorkspacethermal);
}
//! Read in the particle data file and create TParticle_THERMINATOR class object, which is stored in fParticleData vector
void TFastReso_THERMINATOR::read_particles_data(string inputname, bool verbose){
  // Open the input file
  ...
  if (verbose){
      ...
      fParticleData.push_back(move(unique_ptr<TParticle_THERMINATOR>(new TParticle_THERMINATOR(Name, Mass,Gamma,Spin, Isospin, I3, Nq,Ns,Naq,Nas,Nc,Nac,PDGCode))));
    }
  }
  if (verbose) {
...
void TFastReso_THERMINATOR::do_decays(string inputname, bool verbose){
  ...
  if (verbose){
      ...
      if (count==5)
      {
        ...
        if (Ma < Mb+Mc) {
     if (verbose) {     cerr  << "\033[1mTFastReso_THERMINATOR.cpp::do_decays\033[0m : \033[1;31merror\033[0m : father particle lighter than childern! " << Ma << " < " << Mb+Mc << endl;}
          continue;
        }
        ...
        do_2bodydecay(getParticle(Father), getParticle(Child1), getParticle(Child2), BranchingRatio);
        if (Child1!=Child2){ do_2bodydecay(getParticle(Father), getParticle(Child2), getParticle(Child1), BranchingRatio); }
      }
      else if (count==6)
      {
        double Ma = getParticle(Father)->getM();
        ...
        if (Ma < Mb+Mc+Md) {
       if (verbose) {   cerr  << "\033[1mTFastReso_THERMINATOR.cpp::do_decays\033[0m : \033[1;31merror\033[0m : father particle lighter than childern! " << Ma << " < " << Mb+Mc+Md << endl;}
          continue;
        }
        do_3bodydecay(getParticle(Father), getParticle(Child1), getParticle(Child2),getParticle(Child3), BranchingRatio);
        if (Child1!=Child2){ do_3bodydecay(getParticle(Father), getParticle(Child2), getParticle(Child1),getParticle(Child3), BranchingRatio); }
        if (Child1!=Child3 and Child2!=Child3){ do_3bodydecay(getParticle(Father), getParticle(Child3), getParticle(Child1),getParticle(Child2), BranchingRatio); }
      }
    }
  }
  if (verbose) {
